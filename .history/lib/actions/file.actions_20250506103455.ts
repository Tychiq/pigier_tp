...
> => {
  ...
  try {
    ...
    // 2. Check for existing file with same size by same user
    const existingFilesBySize = await databases.listDocuments(
      appwriteConfig.databaseId,
      appwriteConfig.filesCollectionId,
      [
        Query.equal("owner", ownerId),
        Query.equal("size", file.size),
      ]
    );

    if (existingFilesBySize.documents.length > 0) {
      return {
        success: false,
        message:
          "Un fichier avec la même taille existe déjà. Supprimez-le avant d'en importer un nouveau.",
      };
    }
